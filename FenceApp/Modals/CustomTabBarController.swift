...
extension CustomTabBarController: UITabBarControllerDelegate, UIImagePickerControllerDelegate & UINavigationControllerDelegate  {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        guard let currentUser = CurrentUserInfo.shared.currentUser else { return }
        
        Task {
            do {
                let url = try await FirebaseImageUploadService.uploadeFoundImage(image: image)
                let foundDTD = FoundResponseDTO(latitude: latitude, longitude: longitude, imageURL: url, date: Date(), userIdentifier: currentUser.userIdentifier, userProfileImageURL: currentUser.profileImageURL, userNickname: currentUser.nickname)
