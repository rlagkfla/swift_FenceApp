...
private extension LoginViewController {
    ...
    func fetchAndStoreCurrentUser(identifier: String) async {
        do {
            ...
            let fbUser = FBUser(email: user.email, profileImageURL: user.profileImageURL, userIdentifier: user.identifier, nickname: user.nickname, reportCount: user.reportCount)
